...
export function useAIAssistantEnhanced() {
  ...
  const [onFileCreate, setOnFileCreate] = useState<
    ((name: string, content: string, type?: string) => void) | null
  >(null);
  const [onFileUpdate, setOnFileUpdate] = useState<
    ((name: string, content: string) => void) | null
  >(null);
    ...
    try {
      ...
      // Fallback to legacy settings if no BYOK settings
        ...
        try {
          ...
          finalSettings = {
            ...parsed,
            systemPrompt: parsed.systemPrompt?.includes(
      ...
      if (hasValidConfig(finalSettings)) {
  ...
  const saveSettings = useCallback((newSettings: AISettings) => {
    const enhancedSettings = {
      ...newSettings,
      systemPrompt: newSettings.systemPrompt?.includes(
        'White Rabbit Code Editor'
      )
    ...
    try {
      if (typeof window !== 'undefined') {
        localStorage.setItem(
          'byok-ai-settings',
          JSON.stringify(enhancedSettings)
        );
    ...
    if (hasValidConfig(enhancedSettings)) {
      setIsConfigured(true);
  ...
  const parseFileCommands = useCallback((content: string) => {
    ...
    const createMatches = content.match(
      /CREATE_FILE:([^\n]+)\n```[\w]*\n([\s\S]*?)\n```/g
    );
    if (createMatches) {
  ...
  const executeFileCommands = useCallback(
    (
      commands: Array<{
        type: 'create' | 'update';
        filename: string;
  ...
  const sendMessage = useCallback(
    async (
      ...
      if (!aiService || !isConfigured) {
        throw new Error(
          'AI service not configured. Please set up your API key in Settings.'
        );
      }
...
Please help me with this request by refining the existing code.`;
        ...
        if (fileCommands.length > 0) {
          ...
          response.content =
            cleanContent ||
            "I've created/updated the files in your editor. Check the file tabs to see the changes!";
        }
        ...
        for await (const chunk of aiService.sendMessageStream(
          ...
          while (consumed < buffer.length) {
            if (!currentFile) {
              ...
              const headerPos =
                consumed + (buffer.slice(consumed).search(headerRe) ?? 0);
              consumed = headerPos + headerLen;
              currentFile = {
                type: kind === 'CREATE_FILE' ? 'create' : 'update',
                name: filename.trim(),
          ...
          try {
            ...
            if (
              ...
              now - lastCheckAt >= CHECK_THROTTLE_MS
            ) {
...
Return only the JSON object, no additional text.`;
  ...
  const getCachedDocumentation = useCallback(
    (fileName: string): DocumentationData | null => {
      try {
        // Check if we're in the browser environment
  ...
  return {
    ...
    // File generation callbacks
    setFileGenerationCallbacks: useCallback(
      (callbacks: {

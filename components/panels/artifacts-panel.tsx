    ...
    <div className={`ios-glass-surface flex flex-col h-full ${className}`}>
      ...
      <div className="ios-glass-panel-header flex items-center justify-between px-3 py-1.5">
        <div className="flex items-center gap-1.5 text-xs font-medium text-foreground/70">
          ...
          <span>Artifacts</span>
          {artifacts.length > 0 && (
            <span className="text-foreground/45 ml-0.5">({artifacts.length})</span>
          )}
        </div>
      </div>
        ...
        <div className="px-2 py-2 space-y-1">
          ...
          {artifacts.map((a) => {
            ...
            return (
              <div
                key={a.id}
                className="ios-glass-list-item group flex items-center gap-2 px-2 py-1.5 cursor-pointer"
                onClick={() => onSelect?.(a)}
              >
                ...
                {isEditing ? (
                  <Input
                    ...
                    className="ios-glass-input h-6 text-xs flex-1"
                  />
                ) : (
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-foreground/88 truncate">{a.title}</p>
                ...
                <span className="flex items-center gap-0.5 shrink-0 opacity-0 group-hover:opacity-60">
                  <Download
                    className="w-3 h-3 hover:!opacity-100 cursor-pointer text-foreground/60"
                    onClick={(e) => {
                      e.stopPropagation();
                      handleDownload(a);
                    }}

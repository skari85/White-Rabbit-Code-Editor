...
const LiveCodingEngine: React.FC<LiveCodingEngineProps> = ({
  ...
  // Start minimized on narrow viewports so this bar doesn't compete for
  // space with other bottom-anchored controls on mobile.
  ...
  const addTask = useCallback(
    (type: 'create' | 'update', filename: string, content: string) => {
  ...
  const executeLiveCoding = useCallback(
    async (task: CodingTask) => {
      setCurrentTask(task);
      setTasks(prev =>
        prev.map(t => (t.id === task.id ? { ...t, status: 'typing' } : t))
      );
  ...
  return (
    <div
      className={`bg-gray-800/95 backdrop-blur-sm border-t border-gray-600 transition-all duration-300 ${
      ...
      <div className='flex items-center justify-between p-3 bg-gray-700/50'>
        <div className='flex items-center gap-2'>
          ...
          {tasks.length > 0 && !isMinimized && (
            <Badge
              variant='outline'
              className='text-xs px-1.5 py-0.5 text-gray-300'
            >
              {tasks.length} tasks
            </Badge>
          )}
        ...
        <div className='p-3 pt-0 overflow-y-auto max-h-64'>
            ...
            <div className='mb-3 p-2 bg-blue-900/20 border border-blue-500/30 rounded'>
              <div className='flex items-center gap-2 mb-1'>
                ...
                <span className='text-blue-300 text-xs font-medium'>
                  {currentTask.type === 'create' ? 'Creating' : 'Updating'}:{' '}
          ...
          <div className='space-y-1'>
            ...
            {completedTasks.slice(-2).map(task => (
              <div
                key={task.id}
                className='flex items-center gap-2 p-1.5 bg-gray-700/20 rounded opacity-60 text-xs'
              >
                <div
                  className={`w-1.5 h-1.5 rounded-full ${getStatusColor(task.status)}`}
                />

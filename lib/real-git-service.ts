...
export class RealGitService {
  ...
  // Get commit log
  async log(
    options: { depth?: number; since?: Date } = {}
  ...
  async listBranches(): Promise<GitBranchInfo[]> {
    try {
      ...
      const trackingRemote = this.remotes.has('origin')
        ? 'origin'
        : this.remotes.keys().next().value;
          ...
          if (trackingRemote) {
            try {
              const remoteOid = await git.resolveRef({
                fs: this.fs,
                dir: this.dir,
                ref: `refs/remotes/${trackingRemote}/${branch}`,
              });

              const counts = await this.countAheadBehind(oid, remoteOid);
              ahead = counts.ahead;
              behind = counts.behind;
            } catch (error) {
              // No remote-tracking ref for this branch (never fetched) - leave at 0/0
            }
          }
  ...
  // Count commits the local branch is ahead/behind its remote-tracking counterpart
  async countAheadBehind(
    ...
    const countCommitsUntil = async (tip: string): Promise<number> => {
      if (!mergeBase || tip === mergeBase) return 0;
      ...
      const index = commits.findIndex(commit => commit.oid === mergeBase);
      return index === -1 ? commits.length : index;
    };

    const [ahead, behind] = await Promise.all([
      countCommitsUntil(localOid),
      countCommitsUntil(remoteOid),
    ]);

...
import { TerminalComponent, TerminalQuickActions } from "@/components/terminal"
...
export default function Component() {
  ...
  const handleSettingsChange = useCallback((key: string, value: string) => {
    // Generic settings handler for future use
    console.log('Settings changed:', key, value);
  }, []);

  // Enhanced AI integration with terminal
  const handleSendMessage = useCallback(async (message: string) => {
    try {
      const context = {
        files: files,
        selectedFile: selectedFile,
        currentFileContent: getSelectedFileContent(),
      ...
      if (response) {
        // Try to parse and apply any code changes from AI response
        const changesApplied = parseAndApplyAIResponse(response);
        if (changesApplied > 0) {
  ...
  }, [sendAIMessage, files, selectedFile, getSelectedFileContent, parseAndApplyAIResponse, activeTerminalSession]);
            ...
            <div className="h-full flex flex-col">
              ...
              <div className="bg-white border-b p-2">
                <TerminalQuickActions 
                  onRunCommand={(cmd) => terminal.runCommand(cmd)}
                  currentDirectory={activeTerminalSession?.workingDirectory || '/Users/georgalbert/pwa-code'}
                />
              </div>
              ...
              <div className="flex-1">
                <TerminalComponent
                  ...
                  onSwitchSession={terminal.switchSession}
